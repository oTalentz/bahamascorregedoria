...
import { useInfractionsOptimized, Infraction } from '@/hooks/useInfractionsOptimized';
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white mt-4 text-lg">Carregando dados otimizados...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="flex items-center space-x-6">
              ...
              <div>
                ...
                <p className="text-blue-200 mt-2 text-lg">
                  Sistema Otimizado de Acompanhamento de Infrações - Polícia das Bahamas
                  <span className="block text-green-200 text-sm mt-1">
                    ✅ Sistema Otimizado - {user?.name || user?.email} ({userRole === 'admin' ? 'Administrador' : 'Membro'})
                  </span>
                  <span className="block text-blue-100 text-xs mt-1">
                    DEBUG: Role = {userRole} | User ID = {user?.id} | RLS Corrigido
                  </span>
                </p>
              </div>
            </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-3 bg-slate-800/95 border-blue-600/60 mb-8 h-14 shadow-xl">
            ...
            >
              ...
              Logs Otimizados ({auditLogs.length})
            </TabsTrigger>
          </TabsList>
              ...
              <CardContent className="p-8">
                <div className="space-y-6">
                  <div className="relative">
                    ...
                    <Input
                      placeholder="Pesquisa otimizada por nome, ID, guarnição, tipo de punição ou registrado por..."
                      value={searchTerm}
